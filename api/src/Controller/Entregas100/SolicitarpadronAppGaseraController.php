...
class SolicitarpadronAppGaseraController extends AppGaseraController {
    ...
    public function solicitarpadron_fn() {
        ...
        // verifica si la tabla de la cual se tomara el padron existe
        // en caso que no exista esta se creara con el nombre de padron_app_<unidad>
        $sQuery = "SELECT COUNT(*) AS existe " .
            "FROM information_schema.tables " .
            "WHERE table_name = ? " .
            "AND table_schema = ?";
        $aQueryParams = array(
            "padron_app_" . $aUnidad["unidad"],
            $aPlaza["base_te"]
        );
        ...
        $sQuery = "SELECT COUNT(*) AS cantidad FROM padron_app_" . $aUnidad["unidad"];
        ...
        $aPadron = $oConexionPlaza->query($sQuery, $aQueryParams);
