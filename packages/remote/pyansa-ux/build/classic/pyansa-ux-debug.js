...
Ext.define('Pyansa.overrides.form.Panel', {override:'Ext.form.Panel', requires:['Pyansa.overrides.form.field.Text'], trimValues:false, initComponent:function() {
  var me = this, notTrimmedFields, i;
  me.callParent(arguments);
  if (me.trimValues) {
    notTrimmedFields = me.query('[xtype\x3dtextfield]:not([?trimValue])');
    for (i = 0; i < notTrimmedFields.length; i++) {
      notTrimmedFields[i].trimValue = true;
    }
  }
}});
Ext.define('Pyansa.overrides.grid.Panel', {override:'Ext.grid.Panel', requires:['Ext.tip.ToolTip'], showMenuTriggers:false, showMenuHint:false, menuHintText:'Menu para filtrar y ordenar similar a Excel', initComponent:function() {
  var me = this;
  me.callParent(arguments);
  me.addMenuTriggers();
  me.addMenuHint();
}, addMenuTriggers:function() {
  var me = this, columns, i;
  if (me.showMenuTriggers) {
    columns = me.query('gridcolumn[menuDisabled\x3dfalse]:not([?showMenuTrigger])');
    for (i = 0; i < columns.length; i++) {
      columns[i].showMenuTrigger = true;
    }
  }
}, addMenuHint:function() {
  var me = this, firstColumn;
  if (me.showMenuHint) {
    me.addListener('afterlayout', function() {
      firstColumn = me.query('gridcolumn[menuDisabled\x3dfalse][showMenuTrigger]')[0];
      if (firstColumn) {
        Ext.create('Ext.tip.ToolTip', {html:me.menuHintText, target:firstColumn, anchor:true, autoShow:true, defaultAlign:'tl-br', dismissDelay:5000, hideAction:'destroy'});
      }
    }, me, {single:true});
  }
}});
...
Ext.define('Pyansa.overrides.grid.plugin.RowEditing', {override:'Ext.grid.plugin.RowEditing', syncAfterEdit:false, completeEdit:function(config) {
  var me = this, store = me.context.store;
  me.callParent(arguments);
